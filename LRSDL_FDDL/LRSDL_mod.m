function [D, D0, X, X0, W, CoefM, coefM0, opts, rt] = LRSDL_mod(Y, train_label, opts)
% function [D, D0, X, X0, CoefM, coefM0, opts] = LRSDL_mod(Y, train_label, opts)
...
% modified by S.-J.Kim for Asilomar'18
    ...
    %SK: [D, D0, X, X0] = LRSDL_init(Y, Y_range, D_range, optsinit);  
    [D, D0, X, X0] = LRSDL_init(Y, [0 Y_range(end)], D_range, optsinit);
    
    %SK: added: Initialize W
    if opts.Q <= 0
        W = eye(opts.k);
    else
        [U,~,~] = svd(X);
    ...
    if opts.verbose
        %SK: cost_old = LRSDL_cost(Y, Y_range, D, D0, D_range, X, X0, opts);
        cost_old = LRSDL_cost_mod(Y, Y_range, D, D0, D_range, X, X0, W, opts);
    ...
    while iter < opts.max_iter
        ...
        optsX.k0 = opts.k0;
        %SK: [X, X0] = LRSDL_updateXX0(Y, Y_range, D, D_range, D0, X, X0, optsX);
        [X, X0] = LRSDL_updateXX0_mod(Y, Y_range, D, D_range, D0, X, X0, W, optsX);
        ...
        optsD.k0 = opts.k0;
        %SK: D = LRSDL_updateD_fast(Y, Y_range, D, D_range, D0, X, X0, optsD);
        D = LRSDL_updateD_fast_mod(Y, Y_range, D, D_range, D0, X, X0, optsD);
        %% ========= Update D0 ==============================
        if opts.k0 > 0
            if opts.verbose
                fprintf('updating D0...');
...
%             D0 = LRSDL_updateD0(Y, D, D0, Y_range, D_range, opts, X, X0, optsD);
            %SK: D0 = LRSDL_updateD0(Y, Y_range, D, D_range, D0, X, X0, optsD);
            D0 = LRSDL_updateD0_mod(Y, Y_range, D, D_range, D0, X, X0, optsD);
        end
        ...
        if opts.Q > 0
            ...
            W = LRSDL_mod_updateW(X, Y_range, opts.Q);
        end
        
        %%
        if opts.verbose
            if opts.show_cost
                %SK: cost_new = LRSDL_cost(Y, Y_range, D, D0, D_range, X, X0, opts);
                cost_new = LRSDL_cost_mod(Y, Y_range, D, D0, D_range, X, X0, W, opts);
%               t = toc;
