function [StdT, RunCorrT, DLsig, BestRun, IndexAssign, Tmaps] = DL_bestRunSelection(DLsigR)
%% Select best run based on MST.
% Inputs:
% 1. DLsigR - DL signals in a cell array of length equal to the number of ICA runs
...
% Modified for Dictionary Learning by Krishna. Please contact me at krishna5@umbc.edu

%% Initialize vars
numOfRuns = length(DLsigR);
numOfC = size(DLsigR{1}, 1);
numOfVoxels = size(DLsigR{1}, 2);
numOfSub = size(DLsigR{1}, 3);

%% When using IVA stack subjects along voxel dimension
if (numOfSub > 1)
    for nR = 1:numOfRuns
        DLsigR{nR} = reshape(DLsigR{nR}, numOfC, numOfVoxels*numOfSub);
    end
end

[StdT, Tmaps, IcSortMag, IndexAssign] = TmapsMultiRuns(DLsigR);

RunCorrT = zeros(numOfC, numOfRuns);
for k = 1:numOfRuns
    RunCorrT(:, k) = diag(abs(icatb_corr(IcSortMag{k}', Tmaps')));
end
RunCorrT(isnan(RunCorrT))=0;

sum_RunCorrT = sum(RunCorrT);
plot(sum_RunCorrT);
xlabel("Run Index");
ylabel("Sum of Row Correlation With Tmap");
[~, BestZIndex] = max(sum_RunCorrT);

[~, BestRun] = max(sum(RunCorrT));

DLsig = DLsigR{BestRun};

% if (numOfSub > 1)
%     icasig = reshape(icasig, numOfC, numOfVoxels, numOfSub);
% %     A = zeros(numOfIC, numOfIC, numOfSub);
%     A = zeros(numOfC, numOfSub);
%     W = A;
%     for nSub = 1:numOfSub
...
function [StdT, Tmaps, dlSort, IndexAssign] = TmapsMultiRuns(DLsigR)
    % % Components from each are clustered
    [IndexAssign, dlSort] = ComponentAlignment(DLsigR);
    
    NumRun = length(DLsigR);
    NumComp = size(DLsigR{1}, 1);
    voxels = size(DLsigR{1}, 2);
    
    for k = 1:NumRun
    %     sum(isnan(dlSort{k}(:)))
        dlSort{k} = icatb_zscore(dlSort{k}')';
        dlSort{k}(isnan(dlSort{k}))=0;
    end
    
    StdT = zeros(NumComp, voxels);
    ...
    for i = 1:NumComp
        ...
        for j = 1:NumRun
            icSrun = dlSort{j};
            v = icSrun(i,:);
           if sum(isnan(v))>0 || sum(v)~=0 %Fix: ignore Nan components for recentering/skew correctionn
            %BUG: Edge vector must be monotonically non-decreasing caused by
            %Nan values in v
...
function [IndexAssign, DLsigR] = ComponentAlignment(DLsigR)
...
for i=1:size(C, 1)
    %corrM = icatb_corr(DLsigR{1,C(i,1)}', DLsigR{1,C(i,2)}'); %With sign correction 
    corrM = abs(icatb_corr(DLsigR{1,C(i,1)}', DLsigR{1,C(i,2)}'));
    corrM(corrM > 1) = 1;
    [assignment{1,i}, cost(1,i)] = assignmentoptimal(1 - corrM);
    %BUG:RESOLVED(unequal component sizes present in input (DLsigR).
    %ISSUE: assignmentoptimal sometimes results less than the max num of
    %component assignment. error manifests in line 159 assignpair function
end
...
for k = 1:NumRun
    ...
    zeroInd=find(tmpInd==0);
    tmpInd(zeroInd)=zeroInd;% Temp fix 1
    %BUG infinite distance between components returns as zero index which is not a valid matlab index. 
    %Temp Fix1: Leave them in the same indexes.: doesnt work
    %temp Fix 2: remove those elements from Dlsig R
    %Temp Fix 3: zero those elements from Dlsig R after aligning 
%     DLsigR{k}(find(tmpInd==0),:)=[]; %temp fix 2
    DLsigR{k} = DLsigR{k}(tmpInd, :);

    DLsigR{k}(zeroInd,:)=zeros(size(zeroInd,1),size(DLsigR{1},2)); % assign zeros to those components
end
