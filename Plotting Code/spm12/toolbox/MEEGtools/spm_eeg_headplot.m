...
if ~isfield(D, 'inv')
    error('Please define a forward model for 3D plotting');
end
...
if ~eegind
    error('Only EEG modality is supported for 3D plotting.');
end

if isfield(D, 'spline_file')
    spline_file = D.spline_file;
else
    
    sens = D.inv{val}.datareg(eegind).sensors;
    
    elocs = [];
    for i = 1:numel(sens.label)
        elocs(i).labels = sens.label{i};
        elocs(i).X = sens.chanpos(i, 1);
        elocs(i).Y = sens.chanpos(i, 2);
        elocs(i).Z = sens.chanpos(i, 3);
    end
    
    spline_file = spm_file(fullfile(D), 'prefix', 'spline_', 'ext', 'spl');
    
    headplot('setup', elocs, spline_file);
    
    D.spline_file = spline_file;save(D);
end
...
headplot(Y(:)', spline_file, 'electrodes', 'on');
...
if isstr(values)
    ...
    if strcmp(values,'setup')
        ...
        transmat  = eye(4);%traditionaldipfit( g.transform ); % arno
