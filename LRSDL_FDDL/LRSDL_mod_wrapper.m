function [acc, rt] = LRSDL_mod_wrapper(Y_train, label_train, Y_test , label_test, ...
                            k, k0, lambda1, lambda2, lambda3, Q)
% function [acc, rt] = LRSDL_mod_wrapper(Y_train, label_train, Y_test , label_test, ...
...
% modified by S.-J.Kim for Asilomar'18
    ...
    opts.verbose      = true;
    opts.tol         = 1e-8;
    opts.Q           = Q;  %SK: added. If Q is set to 0, W is ignored.
    %% Train 
    [D, D0, X, X0, W, CoefM, coefM0, opts, rt] = ...
                    LRSDL_mod(Y_train, label_train, opts);
    X1 = [X; X0];
    ...
    if numel(D0) ~= 0
        ...
        acc1 = LRSDL_mod_pred_GC(Y_test, D, D0, CoefM, coefM0, opts, label_test, ...
            X, label_train, W);
%         fprintf('LC:\n');
