function [D_bar, D_tilde, Z_bar, Z_tilde, cost_diff] = Solve_permutation(D_bar, D_tilde, Z_bar, Z_tilde, Y_range, opts)
    D = [D_bar, D_tilde];
    Z = [Z_bar; Z_tilde];
    ...
    [cost_bar, ~, ~, ~] = h_cost(D_bar', Y_range);
    cost_old = 0.5 * opts.lambda3 * cost_bar + 0.5 * opts.lambda2 * cost_tilde;
    ...
    new_order = zeros(1, K);
    new_order(assign(:,2)) = assign(:,1);   % new_order[j] = original atom that goes to position j
    D = D(:, new_order);
    Z = Z(new_order, :);
    ...
    [cost_tilde, ~, ~, ~] = fisher_cost(D_tilde', Y_range);
    [cost_bar, ~, ~, ~] = h_cost(D_bar', Y_range);
    cost_new = 0.5 * opts.lambda3 * cost_bar + 0.5 * opts.lambda2 * cost_tilde;
...
function [cost_assigned] = cost_k(D, Y_range, K_bar, K_tilde, idx, opts)
    ...
    for k = 1: K_bar
        cost_assigned = [cost_assigned, opts.lambda3* cost];
    end
...
function [cost, Sw, Sb, cost_k] = h_cost(X, Y_range)
    % X: atoms by features
    nClasses = numel(Y_range) - 1;
    m = mean(X,2);
    n_atoms = size(X, 1);
    ...
    for c = 1: nClasses
        ...
        Nc = size(Xc,2);
        M = repmat(m, 1, Nc);
        cost = cost + normF2(Mc - M) - normF2(Xc - Mc);
        Sw = Sw + normF2(Xc - Mc);
        Sb = Sb + normF2(Mc - M);
    end 
    cost_k = zeros(1, n_atoms);
    for k = 1: n_atoms
         ...
         cost_k(k) = cost_k(k) + 2 * normF2(X(k, :));
    end
end 
