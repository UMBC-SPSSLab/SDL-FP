function [D_bar, D_tilde, Z_bar, Z_tilde, opts, cost, costs] = LRSDL_mod2(Y, train_label, opts, init_seed, isTuning, permutate)
    ...
    if opts.isJSTSP
        ...
        opts.H_tilde = 3*eye(N) - 2*H1 + H2;
    end
    opts.H_bar = 2*H1 - H2 + eye(N);
    %% Start main loop    
    iter = 0;
    D_old_tilde = D_tilde;
    D_old_bar = D_bar;
    ...
    while iter < opts.max_iter
        ...
        [D_bar, D_tilde] = LRSDL_updateD_fast_mod2(Y, Y_range, D_bar, D_tilde, Z_bar, Z_tilde, opts);
        if 1 == permutate
            if opts.isJSTSP
                [D_bar, D_tilde, Z_bar, Z_tilde, cost_decrease] = Solve_permutation_JSTSP(D_bar, D_tilde, Z_bar, Z_tilde, Y_range, opts);
            else
                [D_bar, D_tilde, Z_bar, Z_tilde, cost_decrease] = Solve_permutation(D_bar, D_tilde, Z_bar, Z_tilde, Y_range, opts);
            end
        end
        [~, ~, ~, ~, cost] = LRSDL_cost_mod2(Y, Y_range, D_bar, D_tilde, Z_bar, Z_tilde, opts);
        hasZeroMap = false;
        %{%}
        if ~isTuning
           % check Z_bar
           for k = 1: opts.k_bar
               if 0 == sum(abs(Z_bar(k, :)))
                   hasZeroMap = true;
                   Z_bar(k, :) = normrnd(0, 1, [1, size(Z_bar, 2)]);
                   D_bar(:, k) = normrnd(0, 1, [size(D_bar, 1), 1]);
                   D_bar(:, k) = D_bar(:, k)/ norm(D_bar(:, k));
               end 
           end
           % check Z_tilde
           for k = 1: opts.k_tilde
               if 0 == sum(abs(Z_tilde(k, :)))
                   hasZeroMap = true;
                   Z_tilde(k, :) = normrnd(0, 1, [1, size(Z_tilde, 2)]);
                   D_tilde(:, k) = normrnd(0, 1, [size(D_tilde, 1), 1]);
                   D_tilde(:, k) = D_tilde(:, k)/ norm(D_tilde(:, k));
               end 
           end
        end
        
        if abs(cost - cost_old) < 1e-5 && ~hasZeroMap
            break;
        end
        cost_old = cost;
